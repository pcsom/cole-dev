...
OUTPUT_CORPUS_PATH = '/storage/ice-shared/vip-vvk/data/AOT/psomu3/codenas/einspace/einspace_corpus_dedup_embedded.pkl'
OUTPUT_PATH = '/storage/ice-shared/vip-vvk/data/AOT/psomu3/codenas/einspace/nasbench_einspace_comparison.csv'
OUTPUT_PATH_CSV = '/storage/ice-shared/vip-vvk/data/AOT/psomu3/codenas/einspace'
...
# Run robust comparison (only computes missing trials)
print("\n" + "=" * 80)
print("NASBench-201 Robust Comparison: modernbert_large_grammar_code_embedding with pairwise loss, single target, pca 256 vs modernbert_large_pytorch_code_embedding with pairwise loss, single target, pca 256")
print(f"  Sample sizes: {SAMPLE_SIZES}")
print(f"  CV setup: {N_FOLDS}-fold × {N_REPEATS} repeats = {N_FOLDS * N_REPEATS} trials per size")
print("  (Will only compute missing trials, preserves existing results)")
print("=" * 80)
...
results_df = run_comparison(
    embedding1_name='modernbert_large_grammar_code_embedding',
    ...
    dim_reduction_components_embedding1=256,
